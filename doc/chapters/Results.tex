...
This chapter reports the numerical sizing, the outcome of the MATLAB particle-swarm exploration, the ANSYS HFSS full-wave results for the circular patch family, and the final KiCAD board geometry. Values are given directly from the repository state at commit \texttt{f9b39e9} (2026-04-28).
...
\begin{table}[H]
\centering
\caption{Analytical radius versus substrate. Fringing correction from Eq.~\ref{eq:ae} with $h$ as in the KiCAD stack is included.}
\label{tab:radius}
...
The file \texttt{matlab/best.txt}, which records the best particle at the end of the logged run of 2025-10-02, reports:

\begin{quote}
\texttt{Board\_W = 40.351~mm, PatchFrac = 0.1477, FeedFrac = -0.531} \\
\texttt{Gain = -6.97~dBi, $|Z| = 0.3~\Omega$ ($Z = 0.0 + j0.3~\Omega$), RL = 0.00~dB, AR = 2.33~dB}
\end{quote}
...
\begin{itemize}[noitemsep]
    \item \textbf{Rectangular / Patch 2} (\texttt{Patch 2.aedt}, \texttt{99f9ccd}--\texttt{7263f00}): early sweep of a rectangular patch with slot. $S_{11}$ minima were found near 2.3~GHz but with narrow 10~MHz bandwidth and strong sensitivity to $h$, consistent with the cavity bandwidth estimate of 1--2\%.
    \item \textbf{Hexagonal patch} (\texttt{HexagonalPatch.aedt}, \texttt{5b98260}): hexagonal truncation reduced the circumscribed area by $\approx 15\%$ but introduced $\mathrm{TM}_{21}$ coupling that degraded axial ratio above 6~dB in simulation. The geometry was abandoned.
    \item \textbf{Circular patch, single feed} (\texttt{CircularPatch.aedt}, \texttt{fad1ff9}): single probe at $\rho_0 \approx 5$~mm yielded $S_{11} < -14$~dB at 2.305~GHz, gain $\sim 4$~dBi, linear polarization. Used to calibrate the effective radius correction.
    \item \textbf{Circular patch V3, dual coax} (\texttt{CircularPatchV3.aedt}, \texttt{235c732}): two orthogonal probes at $(\pm\rho_0,0)$ and $(0,\pm\rho_0)$ with independent wave ports. This is the geometry exported to KiCAD. With $\rho_0$ tuned to match 50~$\Omega$, simulation shows $S_{11} < -10$~dB for each port in isolation, inter-port coupling below $-18$~dB, and axial ratio below 3~dB when ports are driven in quadrature (see Discussion). The DXF exported for KiCAD (\texttt{KiCAD/CircularPatchV3.dxf}) and the STEP model (\texttt{KiCAD/CircularPatchV3.step}, added \texttt{2f72509}) are direct outputs of this AEDT project.
\end{itemize}
...
The final board (\texttt{KiCAD/antennaBoard/antennaBoard.kicad\_pcb}, rev. \texttt{5554f5c}) contains three footprints: \texttt{ANT1} (Inspirefly:CircularPatchAntenna) at $(157.99, 98.55)$~mm, and two Molex 732511350 U.FL-compatible connectors J1/J2 on F.Cu. An SMA edge-mount (Amphenol 132289) is retained on the schematic for laboratory testing but is not placed on the final panel.
...
The DXF-to-footprint flow was validated by re-importing the generated \texttt{CircularPatchAntenna.kicad\_mod} into the PCB and confirming that the polygon vertices match the DXF within $0.01$~mm. Footprint origins are at the patch centre; placement at $(157.99, 98.55)$~mm centres the patch on the 50~mm board.
